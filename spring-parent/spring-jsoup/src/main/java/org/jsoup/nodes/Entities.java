...
public class Entities {
	...
	static void escape(StringBuilder accum, String string, Document.OutputSettings out, boolean inAttribute,
		...
		for (int offset = 0; offset < length; offset += Character.charCount(codePoint)) {
			...
			// surrogate pairs, split implementation for efficiency on single
			// char common case (saves creating strings, char[]):
	...
	private static final Object[][] xhtmlArray = { { "quot", 0x00022 }, { "amp", 0x00026 }, { "lt", 0x0003C },
			{ "gt", 0x0003E } };

	static {
		xhtmlByVal = new HashMap<Character, String>();
		base = loadEntities("entities-base.properties"); // most common /
															// default
	...
	private static Map<String, Character> loadEntities(String filename) {
		...
		try {
			// InputStream in = Entities.class.getResourceAsStream("/" +
			// filename);
			// InputStream in =
			...
			throw new MissingResourceException("Error loading entities resource: " + e.getMessage(), "Entities",
					filename);
		}
