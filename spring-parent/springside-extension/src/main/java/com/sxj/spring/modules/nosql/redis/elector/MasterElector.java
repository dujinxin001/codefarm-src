...
public class MasterElector implements Runnable
{
    ...
    public void start(ScheduledExecutorService scheduledThreadPool)
    {
        ...
        electorJob = scheduledThreadPool.scheduleAtFixedRate(new WrapExceptionRunnable(
                this),
                0,
                intervalSecs,
                TimeUnit.SECONDS);
        logger.info("masterElector for {} start, hostName:{}.",
                masterKey,
                hostId);
    }
    ...
    public void stop()
    {
        ...
        {
            Threads.normalShutdown(internalScheduledThreadPool,
                    5,
                    TimeUnit.SECONDS);
        }
    }
            ...
            jedisTemplate.execute(new JedisActionNoResult()
            {
                ...
                public void action(Jedis jedis)
                {
                    ...
                    if (masterFromRedis == null)
                    {
                        ...
                        if (JedisUtils.isStatusOk(jedis.set(masterKey,
                                hostId,
                                "NX",
                                "EX",
                                expireSecs)))
                        {
                            master.set(true);
                            logger.info("master {} is changed to {}.",
                                    masterKey,
                                    hostId);
                            return;
                        }
