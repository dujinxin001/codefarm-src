...
public class ClassLoaderUtil {
	public static InputStream getResourceAsStream(String configFile) throws IOException {
		...
		InputStream configStream = ResourcePatternUtils
				.getResourcePatternResolver(new PathMatchingResourcePatternResolver()).getResource(configFile)
				.getInputStream();
		if (configStream == null)
			throw new FileNotFoundException("Cannot find "
					+ ClassLoaderUtil.class.getClassLoader().getResource(configFile).toExternalForm() + " !!!");
		return configStream;
	}
