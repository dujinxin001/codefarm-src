...
public class ForwardedX509AuthenticationFilter extends AuthenticatingFilter
{
            ...
            ServletResponse response) throws Exception
    {
        ...
        if (!useCertificate && !useSubjectDN && !useIssuerDN
                && !useSerialNumber)
        {
        ...
        if (false)
        {
            ...
            if (!verifiedHeader.isEmpty())
            {
                ...
                {
                    throw new AuthenticationException(
                            "Client certificate verification failure was forwarded");
                }
            }
        }
        ...
        if (useCertificate)
        {
            ...
            {
                throw new AuthenticationException("Set up to use "
                        + SSL_CLIENT_CERT
                        + " header but it was either empty or unparseable");
            }
            
            return new X509AuthenticationToken(certificateChain,
                    getHost(request));
        ...
        {
            throw new AuthenticationException(
                    "All set up forwarded headers were empty");
        }
        
        return new X509AuthenticationToken(subjectDN, issuerDN,
    ...
    {
        httpHeaderValue = httpHeaderValue.replaceAll(PEM_BEGIN_CERT,
                TEMP_BEGIN_CERT);
        httpHeaderValue = httpHeaderValue.replaceAll(PEM_END_CERT,
                TEMP_END_CERT);
        httpHeaderValue = httpHeaderValue.replaceAll("\\s", "\n");
        httpHeaderValue = httpHeaderValue.replaceAll(TEMP_BEGIN_CERT,
                PEM_BEGIN_CERT);
        httpHeaderValue = httpHeaderValue.replaceAll(TEMP_END_CERT,
                PEM_END_CERT);
    ...
    private X509Certificate[] readX509CertificateChainFromPEM(String pem)
    {
        try
        {
            List<X509Certificate> pemBundle = loadPEMBundle(new StringReader(
                    pem));
    ...
    private static List<X509Certificate> loadPEMBundle(
            final Reader pemBundleReader) throws IOException,
                ...
                if (line.length() > 0)
                {
                    ...
                    else if (line.startsWith(PEM_END))
                    {
                        if (begin || !end)
                        {
                            ...
                            certList.add((X509Certificate) certFactory.generateCertificate(new ByteArrayInputStream(
                                    base64DecodedCert)));
                        }
                        else
                        {
    ...
    private X500Principal readX500PrincipalFromString(String dn)
    {
        ...
        {
            LOGGER.warn("Unparseable DN in header string, will use null and continue. Here is the header string: {}",
                    dn,
                    ex);
            return null;
        }
