...
public class HierarchicalCacheManager {
	...
	public void initCacheProvider(String configFile, CacheExpiredListener listener) {
		initCacheProvider(configFile, 1, listener);
	}

	public void initCacheProvider(String configFile, int databaseId, CacheExpiredListener listener) {
		final String path = getConfigFile() == null ? configFile : getConfigFile();
		...
		try {
			...
			props.setProperty("redis.database", props.getProperty("redis.database", String.valueOf(databaseId)));
			configStream.close();
			if (props.getProperty(L1_PROVIDER_CLASS) == null && props.getProperty(L2_PROVIDER_CLASS) == null)
				throw new CacheException("At lease one provider_class should be defined!");
	...
	private void startL2Provider(Properties props)
		...
		if (props.getProperty(L2_PROVIDER_CLASS) != null) {
			...
			HierarchicalCacheManager.l2_provider
					.start(getProviderProperties(props, HierarchicalCacheManager.l2_provider));
			LOGGER.info("Using L2 CacheProvider : " + l2_provider.getClass().getName());
		}
	}
	...
	private void startL1Provider(Properties props)
		...
		if (props.getProperty(L1_PROVIDER_CLASS) != null) {
			...
			HierarchicalCacheManager.l1_provider
					.start(getProviderProperties(props, HierarchicalCacheManager.l1_provider));
	...
	private static final Properties getProviderProperties(Properties props, CacheProvider provider) {
		...
		while (keys.hasMoreElements()) {
			...
			if (key.startsWith(prefix))
				tmp.setProperty(key.substring(prefix.length()), props.getProperty(key));
		}
		return tmp;
	}

	private static final Cache getCache(CacheLevel level, String cacheName, boolean autoCreate) {
		switch (level) {
		case EHCACHE:
		...
		default:
			return new NullCacheProvider().buildCache(cacheName, autoCreate, listener);
		}
	}

	public static void startSubscribe(String channelsName, JedisPubSub pubSub) {
		RedisCacheProvider provider =null;
		boolean broken = false;
		Jedis cache=null;
		try {
			if (l2_provider instanceof RedisCacheProvider) {
				provider = (RedisCacheProvider) l2_provider;
				cache=provider.getResource();
				cache.subscribe(pubSub, channelsName);
			} else {
	...
	public static void startPsubscribe(String channelsName, JedisPubSub pubSub) {
		RedisCacheProvider provider =null;
		boolean broken = false;
		Jedis cache=null;
		try {
			if (l2_provider instanceof RedisCacheProvider) {
				provider = (RedisCacheProvider) l2_provider;
				cache=provider.getResource();
				cache.psubscribe(pubSub, channelsName);
			} else {
				LOGGER.error("缓存管理器没有实现消息队列");
			}
		} catch (Exception e) {
	...
	public static final void publish(CacheLevel level, String name, String message) {
		if (name != null && message != null) {
			Cache cache = getCache(level, "", true);
			if (cache != null)
				cache.publish(name, message);
		}
	}
	...
	public static final <T> T get(CacheLevel level, Class<T> resultClass, String name, Object key) {
		if (name != null && key != null) {
			Cache cache = getCache(level, name, false);
			if (cache != null) {
	 ...
	 * @param value
	 */
	public static final void set(CacheLevel level, String name, Object key, Object value) {
	...
	public static final void set(CacheLevel level, String name, Object key, Object value, int seconds) {
		if (name != null && key != null && value != null) {
			Cache cache = getCache(level, name, true);
			if (cache != null)
				if (seconds <= 0)
	...
	public static final void setNoSeri(CacheLevel level, String name, String key, String value) {
		if (name != null && key != null && value != null) {
			Cache cache = getCache(level, name, true);
			if (cache != null)
	...
	public static final void setNoSeri(CacheLevel level, String name, String key, String value, int seconds) {
		if (name != null && key != null && value != null) {
			Cache cache = getCache(level, name, true);
			if (cache != null)
				if (seconds <= 0)
	 ...
	 * @param seconds
	 */
	public static final byte[] rpop(CacheLevel level, String name, Object key) {
		if (name != null && key != null) {
			Cache cache = getCache(level, name, true);
			if (cache != null)
	 ...
	 * @param seconds
	 */
	public static final Object brpop(CacheLevel level, String name, Object key, int seconds) {
		if (name != null && key != null) {
			Cache cache = getCache(level, name, true);
			if (cache != null)
	 ...
	 * @return
	 */
	public static final void lpush(CacheLevel level, String name, Object key, byte[] value) {
		if (name != null && key != null && value != null) {
			Cache cache = getCache(level, name, false);
			if (cache != null)
