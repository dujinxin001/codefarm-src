...
import com.sxj.ca.interf.Store;
...
public class CAManager
{
    ...
    public void init(Store<KeyPair> keystore, Store<X509Certificate> certstore,
            X509Attrs principals) throws KeyPairException, CertificateException
    {
    ...
    public KeyStore generatePKCS12(X509Certificate[] chain, KeyPair keypair)
            throws CertificateException
    {
        try
        {
    ...
    private KeyPair generateKeyPair(Store<KeyPair> store)
    ...
    private X509Certificate generateCACertificate(KeyPair keyPair,
            Store<X509Certificate> store, X509Attrs principals)
            throws CertificateException
    {
     ...
     */
    private X509Certificate generateCACertificate(KeyPair pair,
            X509Attrs principals) throws InvalidKeyException,
            ...
            boolean isCA) throws CertificateException
    {
        try
        {
            ...
            generator.setNotAfter(new Date(System.currentTimeMillis()
                    + (1000L * 60 * 60 * 24 * days)));
            generator.setSubjectDN(request.getCertificationRequestInfo()
                    .getSubject());
