...
import com.codefarm.fastdfs.monitor.util.JsshProxy;
...
import com.codefarm.fastdfs.monitor.vo.Machine;
import com.codefarm.fastdfs.monitor.vo.Storage;
import com.codefarm.fastdfs.monitor.vo.StorageDay;
...
public class MonitorServiceImpl extends BaseService implements MonitorService
{
    ...
    @Override
    public List<Group> listGroupInfo() throws Exception
    {
    ...
    public List<Line> getNetTrafficLines(String ip, String start, String end)
    {
        ...
        for (int i = 0; i < storages.size(); i++)
        {
            ...
            downLoadLine.getData()
                    .add(new Long[] { created.getTime(), download });
        }
        return lines;
    }
            ...
            throws IOException
    {
        ...
        for (int i = s.size() - 1; i >= 0; i--)
        {
            ...
            sc.getData().add(new Long[] { created.getTime(),
                    s.get(i).getFreeMB() });
        }
        return sc;
    }
    ...
    public List<Line> listStorageLines(String groupName) throws IOException
    {
        ...
        for (Storage s : storages)
        {
            ...
            for (int i = results.size() - 1; i >= 0; i--)
            {
                ...
                line.getData().add(new Object[] {
                        ss.getCreated() == null ? new Date() : ss.getCreated()
                                .getTime(), ss.getMem() });
                line1.getData().add(new Object[] {
                        ss.getCreated() == null ? new Date() : ss.getCreated()
                                .getTime(),
                        Double.parseDouble(ss.getCpu() == null ? "0"
                                : ss.getCpu()) });
            ...
            String endTime) throws IOException
    {
        ...
        try
        {
            ...
            start = (startTime == "" || startTime == null) ? sdf.parse("0000-00-00 00:00")
                    : sdf.parse(startTime);
            end = (endTime == "" || endTime == null) ? sdf.parse(format)
                    : sdf.parse(endTime);
        }
        ...
        for (int i = 0; i <= storages.size() - 1; i++)
        {
            ...
            uploadLine.getData().add(new Long[] { created.getTime(),
                    (totalUpload - u) });
            downLoadLine.getData().add(new Long[] { created.getTime(),
                    (totalDownload - d) });
        }
    ...
    public void saveFile(Fdfs_file f) throws IOException
    {
        Session session = getSession();
        session.save(f);
