...
abstract public class AbstractSingleFormatBirtView extends AbstractUrlBasedView
        implements InitializingBean
{
    ...
    public static interface BirtViewResourcePathCallback
    {
        ...
        String baseImageUrl(ServletContext sc, HttpServletRequest r,
                String reportName) throws Throwable;
        
        String baseUrl(ServletContext sc, HttpServletRequest r,
                String reportName) throws Throwable;
        
        String pathForReport(ServletContext servletContext,
                HttpServletRequest r, String reportName) throws Throwable;
        
        String imageDirectory(ServletContext sc, HttpServletRequest request,
                String reportName);
        
        String resourceDirectory(ServletContext sc, HttpServletRequest request,
                String reportName);
        
        String pathForDocument(ServletContext servletContext,
                HttpServletRequest r, String documentName) throws Throwable;
     ...
     */
    public void setBirtViewResourcePathCallback(
            BirtViewResourcePathCallback birtViewResourcePathCallback)
    {
    ...
    {
        Assert.hasText(this.requestEncoding,
                "the 'requestEncoding' must be set");
        Assert.hasText(this.reportFormatRequestParameter,
                "the 'reportFormatRequestParameter' must not be null");
        Assert.isTrue(StringUtils.hasText(this.reportName)
                || StringUtils.hasText(this.reportNameRequestParameter),
                "the 'reportName' or the 'reportNameRequestParameter' must not be null");
        Assert.hasText(this.documentNameRequestParameter,
                "the 'documentNameRequestParameter' must not be null");
        
        if (null == this.renderOption)
            this.renderOption = new RenderOption();
        
        if (null == this.actionHandler)
            this.actionHandler = new SimpleRequestParameterActionHandler(
                    this.reportNameRequestParameter,
                    this.reportFormatRequestParameter);
        
        if (null == birtViewResourcePathCallback)
            this.birtViewResourcePathCallback = new SimpleBirtViewResourcePathPathCallback(
                    this.reportsDirectory, this.imagesDirectory,
                    this.resourceDirectory, this.documentsDirectory);
    }
    ...
    private String canonicalizeName(String reportName)
    {
        ...
        return !reportName.toLowerCase().endsWith(".rptdesign") ? reportName
                + ".rptdesign" : reportName;
    }
    ...
    private String canonicalizeDocName(String docName)
    {
        ...
        return !docName.toLowerCase().endsWith(".rptdocument") ? docName
                + ".rptdocument" : docName;
    }
    
    @SuppressWarnings("unchecked")
    protected void renderMergedOutputModel(Map<String, Object> modelData,
            HttpServletRequest request, HttpServletResponse response)
            throws Exception
    {
        ...
        try
        {
            ...
            mapOfOptions.put(IModuleOption.RESOURCE_FOLDER_KEY,
                    birtViewResourcePathCallback.resourceDirectory(sc,
                            request,
                            reportName));
            mapOfOptions.put(IModuleOption.PARSER_SEMANTIC_CHECK_KEY,
                    Boolean.FALSE);
            ...
            appContextMap.put(EngineConstants.APPCONTEXT_BIRT_VIEWER_HTTPSERVET_REQUEST,
                    request);
            
            if (this.dataSource != null)
            {
                appContextMap.put(IConnectionFactory.PASS_IN_CONNECTION,
                        this.dataSource.getConnection());
                
                if (this.closeDataSourceConnection)
                    appContextMap.put(IConnectionFactory.CLOSE_PASS_IN_CONNECTION,
                            Boolean.TRUE);
            }
            
            IEngineTask task = null;
            String pathForReport = birtViewResourcePathCallback.pathForReport(sc,
                    request,
                    fullReportName);
            fis = new FileInputStream(pathForReport);
            runnable = birtEngine.openReportDesign(fullReportName,
                    fis,
                    mapOfOptions);
                    ...
                    && this.taskType == AbstractSingleFormatBirtView.RUNRENDERTASK)
            {
                ...
                IRenderOption options = null == this.renderOption ? new RenderOption()
                        : this.renderOption;
                options.setActionHandler(actionHandler);
                IRenderOption returnedRenderOptions = renderReport(modelData,
                        request,
                        response,
                ...
                for (String k : appContextMap.keySet())
                    runAndRenderTask.getAppContext().put(k,
                            appContextMap.get(k));
            ...
            else
            {
                ...
                if (runnable != null)
                {
                    ...
                    String pathForDocument = birtViewResourcePathCallback.pathForDocument(sc,
                            request,
                            fullDocumentName);
                    runTask.run(pathForDocument);
                    runTask.close();
                    document = birtEngine.openReportDocument(fullDocumentName,
                            pathForDocument,
                            mapOfOptions);
            ...
            throws Throwable
    {
        ...
        {
            Assert.isInstanceOf(IScalarParameterDefn.class,
                    param,
                    "the parameter must be assignable to "
                            + IScalarParameterDefn.class.getName());
            IScalarParameterDefn scalar = (IScalarParameterDefn) param;
            if (this.reportParameters != null
                    && this.reportParameters.get(param.getName()) != null)
            {
    ...
    private Object getParamValueObject(HttpServletRequest request,
            IScalarParameterDefn parameterObj) throws Throwable
    {
    ...
    private String getReportParameter(HttpServletRequest request, String name,
            String defaultValue) throws Throwable
    {
        ...
        Set<String> nullParams = getParameterValues(request,
                isNullParameterName);
    ...
    private boolean doesReportParameterExist(HttpServletRequest request,
    ...
    private Set<String> getParameterValues(HttpServletRequest request,
            String parameterName) throws Throwable
    {
        ...
        Collections.addAll(parameterValues,
                null == parameterValuesArray ? new String[0]
                        : parameterValuesArray);
        return parameterValues;
    }
    
    private void handleEncodingInRequest(HttpServletRequest request)
            throws Throwable
    {
...
class SimpleBirtViewResourcePathPathCallback implements
        AbstractSingleFormatBirtView.BirtViewResourcePathCallback
{
    
    private String reportFolder, imagesFolder, resourceFolder, documentsFolder;
    
    public String baseImageUrl(ServletContext sc, HttpServletRequest request,
            String reportName) throws Throwable
    {
        return request.getContextPath() + "/" + imagesFolder;
    }
    
    public String baseUrl(ServletContext sc, HttpServletRequest request,
            String reportName) throws Throwable
    {
    ...
    public String pathForReport(ServletContext sc, HttpServletRequest request,
            String reportName) throws Throwable
    {
    ...
    public String pathForDocument(ServletContext sc,
            HttpServletRequest request, String documentName) throws Throwable
    {
    ...
    public String imageDirectory(ServletContext sc, HttpServletRequest request,
            String reportName)
    {
        return sc.getRealPath(imagesFolder);
    }
    
    public String resourceDirectory(ServletContext sc,
