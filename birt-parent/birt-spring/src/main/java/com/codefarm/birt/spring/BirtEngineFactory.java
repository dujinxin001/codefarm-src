        ...
        ApplicationContextAware, DisposableBean, InitializingBean
{
    ...
    public void setExposedSpringApplicationContextKey(
            String exposedSpringApplicationContextKey)
    {
    ...
    public IReportEngine getObject()
    {
        synchronized (this.monitor)
        {
            ...
            config.getAppContext().put(this.exposedSpringApplicationContextKey,
                    this.context);
            config.setLogConfig(null != this.logDirectory ? this.logDirectory.getAbsolutePath()
                    : null,
                    this.logLevel);
            try
            {
            ...
            {
                throw new RuntimeException("Could not start the BIRT engine.",
                        e);
            }
    ...
    public void afterPropertiesSet() throws Exception
    {
        ...
        {
            Assert.isTrue(logDirectory.isDirectory(),
                    "the path given must be a directory");
            Assert.isTrue(logDirectory.exists(),
                    "the path specified must exist");
        }
        // required properties
