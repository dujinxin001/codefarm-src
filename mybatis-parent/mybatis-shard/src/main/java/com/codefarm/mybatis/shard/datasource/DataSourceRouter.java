package com.codefarm.mybatis.shard.datasource;
...
import com.codefarm.mybatis.shard.datasource.DataSourceFactory.DataSourceNode;
import com.codefarm.mybatis.shard.transaction.ShardManagedTransactionManager;
import com.codefarm.spring.modules.util.RegexUtil;

public class DataSourceRouter
{
    private static final Logger LOGGER = LoggerFactory
            .getLogger(DataSourceRouter.class);
    ...
    public static DataSource getKeyGeneratorDataSource()
    {
        ...
        return keyGeneratorDs
                .get((int) (keyGeneratorDs.size() * Math.random()));
    }
            ...
            BoundSql boundSql, Object param)
    {
        ...
        else
        {
                    ...
                    || commandName.equals("delete"))
            {
                ...
                String whereStr = lowerSql
                        .substring(lowerSql.indexOf("where") + 5);
            ...
            else if (commandName.equals("insert"))
            {
                ...
                String insertValues = sql
                        .substring(lowerSql.indexOf("values") + 6).trim();
            ...
            else
            {
                ...
                {
                    value = HashCodeBuilder.reflectionHashCode(shardValue,
                            true);
                }
                ...
                List<DataSourceNode> availableNodes = DataSourceFactory
                        .getDataNodes(tblName);
                int nodeIndex = value % availableNodes.size();
        ...
        {
            int index = (int) (targetNode.getReadNodes().size()
                    * Math.random());
            if (!isWriteTable(tblName) && ShardManagedTransactionManager
        ...
        {
            int index = (int) (targetNode.getWriteNodes().size()
                    * Math.random());
            ds = targetNode.getWriteNodes().get(index);
            LOGGER.debug("Will Execute in WRITE Mode:|=======" + lowerSql);
        }
