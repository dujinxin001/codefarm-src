...
import com.codefarm.mybatis.shard.datasource.DataSourceRouter;
import com.codefarm.mybatis.shard.mapper.ShardMapperMethod;
...
public class ShardMapperProxy implements InvocationHandler, Serializable
{
            ...
            throws Throwable
    {
        ...
        //        if (ms.isUseCache())
        sqlSession = ShardedSqlSession.getSqlSession(
                ShardSqlSessionFactory.instance(), ExecutorType.REUSE, null);
    ...
    private void processBefore(MappedStatement ms, Object param)
            throws SQLException
    {
        ShardKeyGenerator shardKeyGenerator = GenericStatementBuilder
                .getShardedKeyGenerators().get(ms.getId());
        ShardSnGenerator shardSnGenerator = GenericStatementBuilder
                .getShardSnGenerators().get(ms.getId());
        if (shardSnGenerator != null)
        {
            List<DataSource> snGeneratorDataSources = DataSourceRouter
                    .getSnGeneratorDataSources();
            shardSnGenerator.process(snGeneratorDataSources.get(0),
                    param,
                    ConfigurationProperties.getDialect(
        ...
        if (shardKeyGenerator != null)
        {
            ...
            {
                List<DataSource> keyGeneratorDataSources = DataSourceRouter
                        .getKeyGeneratorDataSources();
                ...
                shardKeyGenerator.process(
                        DataSourceRouter.getKeyGeneratorDataSource(),
                        param,
                        ConfigurationProperties.getDialect(
                                MybatisConfiguration.getConfiguration()));
        }
