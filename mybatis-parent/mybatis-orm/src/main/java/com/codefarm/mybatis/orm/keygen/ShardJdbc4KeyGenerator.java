...
import com.codefarm.mybatis.dialect.IDCfg;
...
public class ShardJdbc4KeyGenerator implements ShardKeyGenerator
{
            ...
            Dialect dialect) throws SQLException
    {
        ...
        try
        {
            Class<?> userClass = com.codefarm.spring.modules.util.Reflections
                    .getUserClass(parameter);
            Field idField = findId(userClass);
            GeneratedValue generatedValue = idField
                    .getAnnotation(GeneratedValue.class);
            String tableName = userClass.getAnnotation(Table.class).name();
            IDCfg idCfg = new IDCfg(generatedValue.table(),
                    generatedValue.idColumn(),
                    generatedValue.delimiterColumn());
