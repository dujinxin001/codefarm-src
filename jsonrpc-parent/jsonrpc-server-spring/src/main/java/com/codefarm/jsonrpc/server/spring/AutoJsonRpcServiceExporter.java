package com.codefarm.jsonrpc.server.spring;
...
public class AutoJsonRpcServiceExporter implements BeanFactoryPostProcessor
{
            ...
            ConfigurableListableBeanFactory beanFactory) throws BeansException
    {
        ...
        for (Entry<String, String> entry : serviceBeanNames.entrySet())
        {
            ...
            registerServiceProxy(dlbf,
                    makeUrlPath(servicePath),
                    serviceBeanName);
        }
    }
            ...
            ConfigurableListableBeanFactory beanFactory)
    {
        ...
        {
            JsonRpcService jsonRpcPath = beanFactory.findAnnotationOnBean(beanName,
                    JsonRpcService.class);
            if (jsonRpcPath != null)
            {
                String pathValue = jsonRpcPath.value();
                LOG.fine(format("Found JSON-RPC path '%s' for bean [%s].",
                        pathValue,
                        beanName));
     ...
     */
    private void registerServiceProxy(DefaultListableBeanFactory dlbf,
            String servicePath, String serviceBeanName)
    {
        ...
        BeanDefinition serviceBeanDefinition = findBeanDefintion(dlbf,
                serviceBeanName);
        for (Class<?> iface : getBeanInterfaces(serviceBeanDefinition,
                dlbf.getBeanClassLoader()))
        {
            if (iface.isAnnotationPresent(JsonRpcService.class))
            {
                String serviceInterface = iface.getName();
                LOG.fine(format("Registering interface '%s' for JSON-RPC bean [%s].",
                        serviceInterface,
                        serviceBeanName));
        ...
        builder.addPropertyValue("backwardsComaptible",
                Boolean.valueOf(backwardsComaptible));
        builder.addPropertyValue("rethrowExceptions",
                Boolean.valueOf(rethrowExceptions));
        builder.addPropertyValue("allowExtraParams",
                Boolean.valueOf(allowExtraParams));
        builder.addPropertyValue("allowLessParams",
                Boolean.valueOf(allowLessParams));
            ...
            ConfigurableListableBeanFactory beanFactory, String serviceBeanName)
    {
        ...
        if (parentBeanFactory != null
                && ConfigurableListableBeanFactory.class.isInstance(parentBeanFactory))
        {
            return findBeanDefintion((ConfigurableListableBeanFactory) parentBeanFactory,
                    serviceBeanName);
        }
        throw new RuntimeException(
                format("Bean with name '%s' can no longer be found.",
                        serviceBeanName));
    }
    
    private Class<?>[] getBeanInterfaces(BeanDefinition serviceBeanDefinition,
