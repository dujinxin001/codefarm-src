...
public class AutoJsonRpcClientProxyCreator implements BeanFactoryPostProcessor,
        ApplicationContextAware
{
            ...
            ConfigurableListableBeanFactory beanFactory) throws BeansException
    {
        ...
        LOG.debug(format("Scanning '%s' for JSON-RPC service interfaces.",
                resolvedPath));
        try
        {
                ...
                if (resource.isReadable())
                {
                    ...
                    if (annotationMetadata.isAnnotated(jsonRpcPathAnnotation))
                    {
                        ...
                        String path = (String) annotationMetadata.getAnnotationAttributes(jsonRpcPathAnnotation)
                                .get("value");
                        boolean useNamedParams = (Boolean) annotationMetadata.getAnnotationAttributes(jsonRpcPathAnnotation)
                                .get("useNamedParams");
                        LOG.debug(format("Found JSON-RPC service to proxy [%s] on path '%s'.",
                                className,
                                path));
                        registerJsonProxyBean(dlbf,
                                className,
                                path,
                                useNamedParams);
                    }
        ...
        catch (IOException e)
        {
            throw new RuntimeException(
                    format("Cannot scan package '%s' for classes.",
                            resolvedPath), e);
        }
    }
    
    /**
     * Converts the scanPackage to something that the resource loader can
     * handle.
     */
    private String resolvePackageToScan()
    {
        return "classpath*:" + convertClassNameToResourcePath(scanPackage)
                + "/**/*.class";
    }
     ...
     */
    private void registerJsonProxyBean(DefaultListableBeanFactory dlbf,
            String className, String path, boolean useNamedParams)
    {
        ...
        ;
        dlbf.registerBeanDefinition(className + "-clientProxy",
    ...
    private String appendBasePath(String path)
    {
        ...
        {
            throw new RuntimeException(
                    format("Cannot combine URLs '%s' and '%s' to valid URL.",
                            baseUrl,
                            path), e);
        }
    }
