...
import com.codefarm.ldap.exception.LpaAnnotationException;

public abstract class LdapEntityHandler implements IAnnotationHandler
{
    private static final org.slf4j.Logger logger = LoggerFactory.getLogger(LdapEntityHandler.class);
            ...
            final Attributes attributes)
    {
        ...
        final String[] supportedClasses = ((LdapEntity) annotation).requiredObjectClasses();
        boolean hasSupportedClass = true;
        for (final String supportedClass : supportedClasses)
        {
            hasSupportedClass = hasSupportedClass
                    && objectClass.contains(supportedClass);
        }
    ...
    { // BEGIN processLdapAttribute()
        ...
        final LdapAttribute attrAnnotation = field.getAnnotation(LdapAttribute.class);
        ...
        { // an aggregate, attribut must be a string
            fieldValue = processAggregate(field,
                    annotatedClass,
                    aggClass,
                    attrAnnotation);
        }
    ...
    protected abstract Object processAttribute(Field field,
            LdapAttribute attrAnnotation) throws NamingException,
    ...
    protected Object processAggregate(final Field field,
            ...
            final LdapAttribute attrAnnotation) throws InstantiationException,
            ...
            InvocationTargetException, NamingException
    {
        ...
        { // BEGIN foreign ldap entry processing for aggregate
            ...
            {
                final Method getDnReference = annotatedClass.getMethod(referenceDNMethod);
            ...
            fieldValue = processForeignAggregate(field,
                    aggClass,
                    dnReference,
                    attrAnnotation);
            ...
            throws InvalidNameException
    {
        ...
        {
            dnLocalReference = dnReference.replace("?", (String) attributeValue);
        }
        else
        {
